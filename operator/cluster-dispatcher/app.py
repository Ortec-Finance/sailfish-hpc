...
class Recv(MessagingHandler):
    ...
    def on_start(self, event):
        ...
        self.conn = (
            ...
            if self.username
            else self.container.connect(url=self.url)
        )
        if self.conn:
            self.receiver = self.container.create_receiver(
                self.conn, source=self.address
            )
    ...
    def on_message(self, event):
        ...
        bestCluster = next(
            (cluster for cluster in clusters if cluster["name"] == targetClusterName),
            None,
        )

        if not self.get_cluster_toleration(targetClusterName):
            print(
                f"The {targetClusterName} Cluster is not tolerating new jobs, Resubmitting job to dispatch and waiting until toleration is lifted."
            )
    ...
    def get_sailfish_crd(self):
        ...
        if len(sc_crd["items"]) == 1:
            ...
            print(
                "Multiple SailfishCluster CRD's detected, please ensure there is only one CRD."
            )
    ...
    def get_sailfish_target_cluster(self):
        sailfish_cluster = self.get_sailfish_crd()
        targetClusterName = sailfish_cluster["status"]["scheduler"][
...
class Send(MessagingHandler):
    ...
    def on_sendable(self, event):
        while event.sender.credit and self.sent < self.total:
            ...
            msg = Message(
                id=(self.sent + 1), body=self.job, durable=self.message_durability
            )
            # sends message
            print(str(msg))
